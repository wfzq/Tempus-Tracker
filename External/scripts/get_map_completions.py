...
def make_api_call(name):
    url = f"https://tempus2.xyz/api/v0/maps/name/{
        name}/zones/typeindex/map/1/records/player/0/3"
    response = requests.get(url)
    if response.status_code == 200:
        try:
            data = response.json()
            completion_info = data.get('completion_info', {})
            completion_info['last_updated'] = datetime.datetime.now(
            ).isoformat()
        ...
        print(f"Failed to fetch data for {
              name}. Status code: {response.status_code}")
    return None
...
def process_batches_and_save(names, output_file_path, batch_size=8):
    ...
    if backup_file_path:
        ...
        print("Unable to make backup, shutting down")
        os._exit(os.EX_OK)
